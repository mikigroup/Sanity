...
function RouterProvider(props) {
  const { onNavigate, router: routerProp, state } = props, resolveIntentLink = React.useCallback(
    ...
    [routerProp, state]
  ), handleNavigateStickyParam = React.useCallback(
    (param, value, options = {}) => {
  ...
  ), [routerState, stickyParams] = React.useMemo(() => {
    if (!state._searchParams)
      return [state, null];
    const { _searchParams, ...rest } = state, [sticky, restParams] = partition__default.default(_searchParams, ([key]) => STICKY_PARAMS.includes(key));
    return sticky.length === 0 ? [state, null] : [{ ...rest, _searchParams: restParams }, sticky];
  }, [state]), router = React.useMemo(
    () => ({
      navigate,
      navigateIntent,
